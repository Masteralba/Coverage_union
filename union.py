import polygon
from polygon import Outline, Node, Polygon
...
def point_in_polygon(x, y, root):
    ...
    while (True):
        ...
        if ((hy < y and ny >= y) or (hy >= y and ny < y)) and (hx <= x or nx <= x) and (
...
def parse_result(result):
    for i in range(len(result)):
        result[i] = result[i][::-1]
    outlines = [Outline(l) for l in result]
    return polygon.Polygon(outlines[0], outlines[1:])
